...
def load_data(cfg: dict) -> (Dataset, Dataset, Optional[Dataset],
    ...
    src_gloss = data_cfg['src_gloss']
    trg_gloss = data_cfg['trg_gloss']
    ...
    src_gloss_field = data.Field(init_token=None, eos_token=EOS_TOKEN,
                           pad_token=PAD_TOKEN, tokenize=tok_fun,
                           batch_first=True, lower=lowercase,
                           unk_token=UNK_TOKEN,
                           include_lengths=True)

    trg_gloss_field = data.Field(init_token=None, eos_token=EOS_TOKEN,
                           pad_token=PAD_TOKEN, tokenize=tok_fun,
                           batch_first=True, lower=lowercase,
                           unk_token=UNK_TOKEN,
                           include_lengths=True)
    ...
    reg_src_field = data.Field(sequential=True,
                               use_vocab=False,
                               dtype=torch.float32,
                               batch_first=True,
                               include_lengths=True,
    ...
    train_data = SignTransRecDataset(path=train_path,
                                    exts=("." + src_lang, "." + trg_lang, "." + src_gloss, "." + trg_gloss, "." + files_lang),
                                    fields=(reg_src_field, reg_trg_field, src_gloss_field, trg_gloss_field, files_field),
                                    trg_size=trg_size,
    ...
    src_vocab_file = data_cfg.get("src_gloss_vocab", None)
    src_gloss_vocab = build_vocab(field="src_gloss", min_freq=src_min_freq,
                            max_size=src_max_size,
                            dataset=train_data, vocab_file=src_vocab_file)
    trg_vocab_file = data_cfg.get("trg_gloss_vocab", None)
    trg_gloss_vocab = build_vocab(field='trg_gloss', min_freq=src_min_freq,
                            max_size=src_max_size,
                            dataset=train_data, vocab_file=trg_vocab_file)
    
    src_vocab = [None] * trg_size
    ...
    dev_data = SignTransRecDataset(path=dev_path,
                                  exts=("." + src_lang, "." + trg_lang, "." + src_gloss, "." + trg_gloss,  "." + files_lang),
                                  trg_size=trg_size,
                                  fields=(reg_src_field, reg_trg_field, src_gloss_field, trg_gloss_field, files_field),
                                  skip_frames=skip_frames)
    ...
    src_gloss_field.vocab = src_gloss_vocab
    trg_gloss_field.vocab = trg_gloss_vocab
    #print(train_data)
    return train_data, dev_data, test_data, src_vocab, trg_vocab, src_gloss_vocab, trg_gloss_vocab
...
class SignTransRecDataset(data.Dataset):
    """Defines a dataset for machine translation of sign videos."""
    ...
    def __init__(self, path, exts, fields, trg_size, skip_frames=1, **kwargs):
        ...
        if not isinstance(fields[0], (tuple, list)):
            fields = [('src', fields[0]), ('trg', fields[1]), ('src_gloss', fields[2]), ('trg_gloss', fields[3]), ('file_paths', fields[4])]

        src_path, trg_path, src_gloss_path, trg_gloss_path,  file_path = tuple(os.path.expanduser(path + x) for x in exts)

        examples = []
        # Extract the parallel src, trg and file files
        with io.open(src_path, mode='r', encoding='utf-8') as src_file, \
                io.open(trg_path, mode='r', encoding='utf-8') as trg_file, \
                io.open(src_gloss_path, mode='r', encoding='utf-8') as src_gloss_file, \
                io.open(trg_gloss_path, mode='r', encoding='utf-8') as trg_gloss_file, \
                    io.open(file_path, mode='r', encoding='utf-8') as files_file:

            i = 0
            # For Source, Target and FilePath
            for src_line, trg_line, src_gloss_line, trg_gloss_line, files_line in zip(src_file, trg_file, src_gloss_file, trg_gloss_file,  files_file):
                i+= 1

                # Strip away the "\n" at the end of the line
                src_line, trg_line, src_gloss_line, trg_gloss_line, files_line = src_line.strip(), trg_line.strip(), src_gloss_line.strip(), trg_gloss_line.strip(),  files_line.strip()
                ...
                src_line = [(float(joint) + 1e-8) for joint in src_line]
                ...
                src_frames = [src_line[i:i + trg_size] for i in range(0, len(src_line), trg_size*skip_frames)]
                
                # Split target into joint coordinate values
                trg_line = trg_line.split(" ")
                #trg_line = trg_line.split(" ")
                 #       [src_frames, trg_frames, files_line], fields))
                ...
                if src_line != '' and trg_line != '':
                    examples.append(data.Example.fromlist(
                        [src_frames, trg_frames, src_gloss_line, trg_gloss_line, files_line], fields))
         #       if len(examples) > 10:
          #          print("TEST ONLY --->")
           #         break
        #print(len(examples))
        super(SignTransRecDataset, self).__init__(examples, fields, **kwargs)

class SignTransDataset(data.Dataset):
    """Defines a dataset for machine translation of sign videos."""
        ...
        with io.open(src_path, mode='r', encoding='utf-8') as src_file, \
            ...
            for src_line, trg_line, files_line in zip(src_file, trg_file, files_file):
                ...
                if len(examples) > 10:
                    ...
                    break
        #print(len(examples))
        super(SignTransDataset, self).__init__(examples, fields, **kwargs)
