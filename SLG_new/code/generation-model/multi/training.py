...
from model import build_translation_model, build_model
...
from model import Model, TranslationModel
...
class TrainManager:
    ...
    def __init__(self, model: Model, model2: TranslationModel,  config: dict, test=False, ckpt=None) -> None:
      ...
      #self.pad_index = self.model.pad_index
        ...
        self.recognition_loss = torch.nn.CTCLoss(
        ...
        self.optimizer = build_optimizer(config=train_config, parameters=list(model.parameters()) + list(model2.parameters()))
        #self.optiizer2 = build_optimizer(config=train_config, parameters=model2.parameters())
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    valid_score, valid_loss, valid_references, valid_hypotheses, \
                        ...
                        validate_on_data(
                            ...
                            recognition_function=self.recognition_loss,
                    ...
                    if self.is_best(ckpt_score):
                        ...
                        display = list(range(0, len(valid_hypotheses), int(np.ceil(len(valid_hypotheses) / 5))))
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> Tensor:
        ...
        batch_loss, noise = self.model.get_loss_for_batch(
            batch=batch, loss_function=self.loss, recognition_loss_function=self.recognition_loss)
        
        translation_loss, _, _, _ = self.model2(return_type="loss",
        ...
        print("for this batch , video loss %.4f  ; gloss translation loss: %s"%(batch_loss, translation_loss))
        ...
        norm_batch_loss = (batch_loss  + translation_loss) / normalizer
        ...
        if self.clip_grad_fun is not None:
            ...
            self.clip_grad_fun(params=self.model2.parameters())
        if update:
            # make gradient step
            self.optimizer.step()
...
def train(cfg_file: str, ckpt=None) -> None:
    ...
    train_data, dev_data, test_data, src_vocab, trg_vocab, src_gloss_vocab, trg_gloss_vocab = load_data(cfg=cfg)
    print("len train data", len(train_data))
    # Build the Progressive Transformer model
    model = build_model(cfg, src_vocab=src_vocab, trg_vocab=trg_vocab, src_gloss_vocab=src_gloss_vocab, trg_gloss_vocab=trg_gloss_vocab)
    translation_model = build_translation_model(cfg, 
                                                src_vocab=src_gloss_vocab,
                                                trg_vocab=trg_gloss_vocab)
    #print(translation_model)
    translation_model.loss_function = ("crossentropy", 0.2)
    #raise EOFError
    if ckpt is not None:
    ...
    trainer = TrainManager(model=model, model2=translation_model, config=cfg)
...
def test(cfg_file,
         ckpt1=None, ckpt2=None, output_fname="default") -> None:
    ...
    if ckpt1 is None:
        ckpt1 = get_latest_checkpoint(model_dir,post_fix="_best")
        if ckpt1 is None:
    ...
    model_checkpoint = load_checkpoint(ckpt1, use_cuda=use_cuda)
    ...
    with open("output_%s.skels"%output_fname, "w") as out_skels:
        # For each of the required data, produce results
        for data_set_name, data_set in data_to_predict.items():
            print("inside for loop data set length:", len(data_set))
            ...
            for sample in hypotheses:
                ...
                for frame in sample:
                    for coord in frame:
                        temp += ('{:.5f}'.format(coord) + " ")
                out_skels.write((temp.strip(' ')+"\n"))
                print("finish1")
...
#     #test(cfg_file=args.config, ckpt=get_latest_checkpoint('./Models/Base_Gloss2P',post_fix="_best"))
#     test(cfg_file=args.config, ckpt=get_latest_checkpoint('./Models/Base_T2P',post_fix="_best"))
  # test(cfg_file="Configs/Base_real.yaml", ckpt="./Models/Base_csl_asl/335000_every.ckpt", output_fname="csl_asl") 
 # test(cfg_file="Configs/Base_asl_csl.yaml", ckpt="./Models/Base_asl_csl/120000_every.ckpt", output_fname="asl_csl")
  #test(cfg_file="Configs/Base_csl_dgs.yaml", ckpt="./Models/Base_csl_dgs/305000_every.ckpt", output_fname="csl_dgs")
